...
const LOGO_PATH = path.join(__dirname, "..", "public", "assets", "bci-logo.png");
...
function sectionBar(doc, title, y, width) {
  doc.rect(MARGIN_X, y, width, SECTION_BAR_HEIGHT).fill(COLORS.navy);
  doc
    .fillColor("#ffffff")
    .font("Helvetica-Bold")
    .fontSize(10)
...
function generatePatientFormPdf(data) {
  return new Promise((resolve, reject) => {
    ...
    doc.rect(MARGIN_X, y, contentWidth, HEADER_HEIGHT).fill(COLORS.navy);
    doc.rect(MARGIN_X, y + HEADER_HEIGHT - 3, contentWidth, 3).fill(COLORS.red);

    let titleX = MARGIN_X + 14;
    try {
      const logoHeight = 32;
      const logoY = y + (HEADER_HEIGHT - 3 - logoHeight) / 2;
      doc.roundedRect(MARGIN_X + 14, logoY - 4, 96, logoHeight + 8, 4).fill("#ffffff");
      doc.image(LOGO_PATH, MARGIN_X + 18, logoY, { height: logoHeight });
      titleX = MARGIN_X + 122;
    } catch (err) {
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fontSize(18)
      .text("Patient Information Form", titleX, y + (HEADER_HEIGHT - 3) / 2 - 9);
    y += HEADER_HEIGHT + 14;
